...
import org.apache.hudi.gcp.bigquery.util.Utils;
...
public class BigQuerySyncTool extends AbstractSyncTool {
  ...
  public void syncHoodieTable() {
    try (HoodieBigQueryClient bigQueryClient = new HoodieBigQueryClient(Utils.propertiesToConfig(props), fs)) {
      switch (bigQueryClient.getTableType()) {
        case COPY_ON_WRITE:
          syncCoWTable(bigQueryClient);
          break;
        case MERGE_ON_READ:
        default:
          throw new UnsupportedOperationException(bigQueryClient.getTableType() + " table type is not supported yet.");
      }
    } catch (Exception e) {
  ...
  private void syncCoWTable(HoodieBigQueryClient bigQueryClient) {
    ValidationUtils.checkState(bigQueryClient.getTableType() == HoodieTableType.COPY_ON_WRITE);
    LOG.info("Sync hoodie table " + snapshotViewName + " at base path " + bigQueryClient.getBasePath());
    ...
    if (!bigQueryClient.tableExists(manifestTableName)) {
      bigQueryClient.createManifestTable(manifestTableName, manifestFileWriter.getManifestSourceUri());
      LOG.info("Manifest table creation complete for " + manifestTableName);
    }
    if (!bigQueryClient.tableExists(versionsTableName)) {
      bigQueryClient.createVersionsTable(versionsTableName, cfg.sourceUri, cfg.sourceUriPrefix, cfg.partitionFields);
      LOG.info("Versions table creation complete for " + versionsTableName);
    }
    if (!bigQueryClient.tableExists(snapshotViewName)) {
      bigQueryClient.createSnapshotView(snapshotViewName, versionsTableName, manifestTableName);
